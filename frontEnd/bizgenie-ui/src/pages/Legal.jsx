      ...
      <div className="space-y-4">
        {legalTasks.length === 0 && (
          <div className="p-6 text-center text-slate-500 bg-white border border-slate-200 rounded-xl">
            Nu există task-uri legale de afișat încă. Trimite o cerere legală pentru a începe.
          </div>
        )}
        {legalTasks.map((task) => (
          <div
                      ...
                      {task.steps &&
                        task.steps.map((step, index) => (
                          <div
                            ...
                            className="flex items-start gap-3 p-3 bg-slate-50 hover:bg-white border border-slate-100 hover:border-blue-200 rounded-xl transition-all cursor-pointer group shadow-sm"
                          >
                            <div className="mt-0.5">
                              {step.done ? (
                                <CheckSquare className="w-5 h-5 text-emerald-500 shrink-0" />
                              ) : (
                                <Square className="w-5 h-5 text-slate-400 group-hover:text-blue-500 shrink-0" />
                              )}
                            </div>
